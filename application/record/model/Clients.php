...
class Clients{
    ...
    public function selectPage($tableName2, $param, $where, $num, $count){
        $client = Db::table($this->tableName)->join($tableName2, $this->tableName.'.'.$param.'='.$tableName2.'.'.$param)->where($where)->paginate($num, $count);
        return $client;
    }
    /**
     * @param $join
     ...
     * @Description 联合查询
     */
    public function joinSelect($joinTable, $param, $where){
        $client = Db::table($this->tableName)->join($joinTable, $this->tableName.'.'.$param.'='.$joinTable.'.'.$param)->where($where)->select();
        return $client;
    }
